...
export default {
  ...
  actions: {
    ...
    async getuseradd(
      { dispatch },
      { username, password, email, mobile, query, pagenum, pagesize }
    ) {
      try {
    ...
    async putusers(
      { dispatch },
      { id, email, mobile, query, pagenum, pagesize }
    ) {
      try {
    ...
    async postcategories(
      { dispatch },
      { cat_pid, cat_name, cat_level, type, pagenum, pagesize }
    ) {
      try {
    ...
    async putcategories(
      { dispatch },
      { id, cat_name, type, pagenum, pagesize }
    ) {
      try {
    ...
    //27.编辑提交参数请求
    async putcategoriesattrId(
      { dispatch },
      { id, attrId, attr_name, attr_sel }
    ) {
      try {
